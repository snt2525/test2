...
document.domain = "http://rootplan-namhee4.4b63.pro-ap-southeast-2.openshiftapps.com/Second.html";

function init(){
	var url = location.href;
	var confmKey = "U01TX0FVVEgyMDE4MDcxMTIxMDYwMTEwODAwMzQ=";//승인키
	...
	if(inputYn != "Y"){
		...
		//부모 클래스에 정보를 보내기
		opener.document.form2.admCd.value = "<%=admCd%>";
		opener.document.form2.rnMgtSn.value = "<%=rnMgtSn%>";
		opener.document.form2.udrtYn.value = "<%=udrtYn%>";
		opener.document.form2.buldMnnm.value = "<%=buldMnnm%>";
		opener.document.form2.buldSlno.value = "<%=buldSlno%>";		
		opener.document.form.jibunAddr.value =  "<%=bdNm%>";
		var sigugun = "<%=sggNm%>";   
        var sigugunArr = sigugun.split(" ");
        opener.document.saveAddress.si.value = "<%=siNm%>";
        if(sigugunArr.length == 1){
        	opener.document.SiData.clickSi.value =  "<%=siNm%>";
        }else{
        	opener.document.SiData.clickSi.value = sigugunArr[0];
        }
	    opener.getAddr(); //주소 넣기	    
